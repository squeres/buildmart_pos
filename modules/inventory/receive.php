...
$products = Database::all(
    "SELECT id, name_en, name_ru, sku, unit, cost_price
     FROM products
     WHERE is_active = 1
     ORDER BY name_en"
);
...
if (is_post()) {
    ...
    if ($qty <= 0) {
        $errors['qty'] = _r('lbl_required');
    }

    if (!$errors) {
        [$qtyBefore, $qtyAfter] = update_stock_balance($pid, $warehouseId, $qty);
        Database::exec(
            "UPDATE products SET cost_price = IF(? > 0, ?, cost_price) WHERE id = ?",
        ...
        <div class="form-group">
          ...
          <div class="product-search-field">
            <div class="product-search-main">
              <select name="product_id" class="form-control" id="inventoryReceiveProductSel" required>
                <option value=""><?= __('lbl_select') ?></option>
                <?php foreach ($products as $product): ?>
                  <option
                    value="<?= (int)$product['id'] ?>"
                    data-cost="<?= e((string)$product['cost_price']) ?>"
                    <?= $productId === (int)$product['id'] ? 'selected' : '' ?>
                  >
                    <?= e(product_name($product)) ?> - <?= e((string)$product['sku']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
            </div>
...
const receiveProductSel = document.getElementById('inventoryReceiveProductSel');
const receiveCameraBtn = document.getElementById('inventoryReceiveCameraBtn');
const unitCostInput = document.getElementById('unitCost');

receiveProductSel.addEventListener('change', function () {
...
if (receiveCameraBtn && window.ProductCameraScanner) {
  window.ProductCameraScanner.attach(receiveCameraBtn, {
    onDetected: async (code) => {
      ...
      if (exact) {
        receiveProductSel.value = String(exact.id);
        receiveProductSel.dispatchEvent(new Event('change', { bubbles: true }));
      }
    }
  });
}
